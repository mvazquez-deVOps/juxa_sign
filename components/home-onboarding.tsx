...
function nextRecommendation(c: Counts): { title: string; body: string; href: string; cta: string } {
  if (c.companies === 0) {
    return {
      title: "Siguiente paso: registrar una empresa",
      body: "Sin IdClient de DIGID no puedes crear firmantes ni documentos. Empieza en Empresas.",
      href: "/empresas/nueva",
  ...
  if (c.signatories === 0) {
    return {
      ...
      body: "Necesitas al menos un firmante por empresa para asignar y enviar a firmar.",
      href: "/firmantes",
      cta: "Ir a firmantes",
    };
  }
  if (c.documents === 0) {
    return {
      ...
      body: "Crea el PDF en DIGID desde el panel y luego coloca las marcas de firma en el visor.",
  ...
  return {
    ...
    body: "En la ficha Enviar sincronizas firmantes con DIGID, ejecutas send_doc y obtienes las URLs.",
    href: "/envios",
    cta: "Envíos y estado",
  };
}

export function HomeOnboarding({ counts }: { counts: Counts }) {
  const rec = nextRecommendation(counts);
  const stepRows = [
    { ok: counts.companies > 0, label: "Empresa registrada en DIGID", href: "/empresas" },
